...
```{r item_categories_price_plot, tidy=TRUE, fig.align='left', fig.width=8}
train[, .(median = median(price)), by = category_name][
  order(median, decreasing = TRUE)][1:30] %>%
...
```{r category_levels, tidy=TRUE}
train[, c("level_1_cat", "level_2_cat",  "level_3_cat",
          "level_4_cat", "level_5_cat") := tstrsplit(train$category_name,
                                                     split = "/")]
head(train[, c("level_1_cat", "level_2_cat", "level_3_cat", "level_4_cat", "level_5_cat")])
```
...
```{r level_2_cat_plot, fig.height=40, fig.width=15}
train %>%
