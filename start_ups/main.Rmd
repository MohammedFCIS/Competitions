...
```{r}
NA_percnt =as.numeric( (count(startups, year_of_founding=="NA"))[2,2] /nrow(startups)*100)
NA_percnt
```
...
outlier_percent <- (outlier_count/nrow(startups))*100
```
...
as.numeric((count(startups, age_of_company_in_years=="NA"))[2,2]/nrow(startups)*100)
```
...
```{r}
as.numeric((count(startups, internet_activity_score=="NA"))[2,2]/nrow(startups)*100)
```
## couning NA based on Company state
```{r }
NA_count <- as.numeric(count(startups[is.na(startups$internet_activity_score), 2]))
```  
We found that `NA_count` of the NA of internet_activity_score is failed, and 1/3 are success
...
## percentage of NA value in internet_activity_score
```{r}
as.numeric((count(startups, employee_count=="NA"))[2,2]/nrow(startups)*100)
```
...
# percentage of NA value in internet_activity_score
```{r}
as.numeric((count(startups, last_funding_amount=="NA"))[2,2]/nrow(startups)*100)
```
...
startups %>%  
  ...
  geom_text(aes(label = outlier), na.rm = TRUE, hjust = -0.3, color="red")
```
this feature needs standardization techiniques in order to change its values.
...
The bar shows that the most succee and failed companies are located in the first section, and the number of seed investor less than 4, and also the NA data for this features seems to be failed , overwhelmingly we can replace the NA with failed company status with median, and the success one with the median of success compnaies. Moreover, around have of this features values is 0, which can indicate that we can convert this feature to categorical one basen on 2 values.
...
```{r}
as.numeric(count(startups[is.na(startups$number_of_investors_in_seed), 2]))
```
...
```{r}
as.numeric(count(startups[is.na(startups$number_of_investors_in_angel_and_or_vc), 2]))
```  
...
## percentage of NA value in internet_activity_score
```{r}
as.numeric((count(startups, 'number_of_co-founders'=="NA")[2,2]/nrow(startups))*100)
```  
...
## percentage of NA value in internet_activity_score
```{r}
(count(startups, number_of_of_advisors=="NA")[2,2]/nrow(startups))*100
```
...
## percentage of NA value in internet_activity_score
```{r}
(count(startups, team_size_senior_leadership=="NA")[2, 2]/nrow(startups))*100
```
...
## list of the values, and their count
```{r}
as.data.frame(count(startups, presence_of_a_top_angel_or_venture_fund_in_previous_round_of_investment))
```
...
We can impement NA value in Decision tree algorith, as it has a big percent.
 
## frequency of the company sattus
```{r}
as.numeric(count(startups[is.na(startups$presence_of_a_top_angel_or_venture_fund_in_previous_round_of_investment), 2]))
```
...
```{r}
as.numeric((count(startups, number_of_of_repeat_investors=="NA")[2, 2]/nrow(startups))*100)
```
...
```{r}
as.numeric((count(startups, number_of__sales_support_material=="NA")[2, 2]/nrow(startups))*100)
```  
...
# Summary of Average size of companies worked for in the past  

```{r}
summary(startups$average_size_of_companies_worked_for_in_the_past)
```
The summary function shows that most of the co-founders and founders had been working in small-size companies before strating up their own business.   

## percentage of NA value 

```{r}
NA_Percent = as.numeric((count(startups, average_size_of_companies_worked_for_in_the_past=="NA")[2,2]/nrow(startups))*100)
```  
There is about `NA_Percent` of the founder and co-founder their previous experience is uknown, either to replace the NA value with appropriate value ofthis feature like no experience, or leave it as it is.

## drawing a bar chart of the feature.  
```{r}
ggplot(startups, aes(x=average_size_of_companies_worked_for_in_the_past, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Average Size of Companies \n worked for in the past", x = "Companies sizes ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
The bar graph shows that,the more companies' size the previous co-founder's and founders expreinece had on, the more likely the start-up status will be success.

# Summary of Have been part of startups in the past?  

```{r}
summary(startups$`have_been_part_of_startups_in_the_past?`)
```
 
This feature indicate that, the overwhelming majority of foundersand co-founders have neen part
in the startup in the past.


## percentage of NA value 

```{r}
NA_Percent = as.numeric((count(startups, `have_been_part_of_startups_in_the_past?`=="NA"))[2,2]/nrow(startups)*100)
```  
There is about `NA_Percent` of the founder and co-founder that, we dont know whether they have
been part of stastup , either to replace the NA value with appropriate value of this feature like no experience, or leave it as it is.

## drawing a bar chart of the feature.  
```{r}
ggplot(startups, aes(x=`have_been_part_of_startups_in_the_past?`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)), position = "dodge")   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Founder & co-founder \n startups in the past", x = "been part ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
The graph shows no clear view weather been part in the past startup will have s success of failed startups in the coming companies.

# Summary of Have been part of successful startups in the past?  

```{r}
summary(startups$`have_been_part_of_successful_startups_in_the_past?`)
```
This feature is highly related to the previos one, therefore wehn we're buidling our model we can shoose one of them


## percentage of NA valuee

```{r}
NA_Percent = as.numeric((count(startups, `have_been_part_of_successful_startups_in_the_past?`=="NA")[2, 2]/nrow(startups))*100)
...
## drawing a bar chart of the feature.  
```{r}
ggplot(startups, aes(x=`have_been_part_of_successful_startups_in_the_past?`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Have been part of successful \n startups in the past", x = "Companies sizes ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
# Summary of Breadth of experience across verticals

```{r}
summary(startups$breadth_of_experience_across_verticals)
```
The summary function shows that most of the co-founders and founders had between low and medium across verticals experirence.   

## percentage of NA value 

```{r}
NA_Percent = (count(startups$breadth_of_experience_across_verticals=="NA")[2,2]/nrow(startups))*100
NA_Percent
```  
There is about `NA_Percent` of the founder and co-founder their `breadth_of_experience_across_verticals` are uknown, either to replace the NA value with appropriate value of this feature like no experience, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=breadth_of_experience_across_verticals, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Experience across verticlas \n", x = "Vertical experience status ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
The graph clearly explanied that, all the founerds and co-founders having high across vertical experience were able to success in their startups companies. while the startups companies staus are fluctuating betweeen the ones havinh low and meduim across vertical experience. Moreover, NA values tend to failed, we can consider this value as No experience.

# Summary of Consulting experience?

```{r}
summary(startups$`consulting_experience?`)
```
...
```{r}
NA_Percent = (count(startups$`consulting_experience?`=="NA")[2,2]/nrow(startups))*100
 
```  
There is about `NA_Percent` of the founder and co-founder`consulting_experience?` is uknown, either to replace the NA value with appropriate value ofthis feature like no experience, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=`consulting_experience?`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Consulting Experience \n", x = "Consulting Experience ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
There is fluctating of the startup-companies status among the founderand co-founders consulting experiencewith sligh increate of success probabilitiesti tose who haven't have consulting experience.

# Summary of Was he or she partner in Big 5 consulting?

```{r}
summary(startups$`was_he_or_she_partner_in_big_5_consulting?`)
```
The summary function shows that most of the co-founders and founders were not working in the big 5 consulting companies.

## percentage of NA value 

```{r}
NA_Percent = (count(startups$`was_he_or_she_partner_in_big_5_consulting?`=="NA")[2,2]/nrow(startups))*100
names(startups)
 
```  
There is about `NA_Percent` of the founder and co-founder working in big 5 consulting companies is uknown, either to replace the NA value with appropriate value ofthis feature like no experience, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=`was_he_or_she_partner_in_big_5_consulting?`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Consulting inbiggest 5 companies \n", x = "Consulting", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
the graph clearly shows that, whoever was a part of top 5 biggest companies, then it's extremely likely his company will be success during the startups.

```{r}
md.pattern(startups[,31:30])
```
This function explain that, to what extent there is storge relevant information between the two features, which will hekp us in dealing with NA values

# Summary of Experience in Fortune 100 organizations

```{r}
startups$experience_in_fortune_100_organizations = as.factor(startups$experience_in_fortune_100_organizations)
summary(startups$experience_in_fortune_100_organizations)
```
most of the cofouners and founers don't have experience in fortune 100 organizations
## percentage of NA value 

```{r}
NA_Percent = (count(startups$experience_in_fortune_100_organizations=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder their previous experience on fortune 100 organizations is uknown, either to replace the NA value with appropriate value ofthis feature like no experience, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=`experience_in_fortune_100_organizations`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Experience in Fortune 100 organization", x = " Experience? ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
The graph clearly explained that, it's extremely like the copnay startups status will be success if its co-founer and founers have experience in Fortune 100 organization, whereas the ones don't have would mostly fail.

# Summary of Experience in Fortune 500 organizations

```{r}
startups$experience_in_fortune_500_organizations = as.factor(startups$experience_in_fortune_500_organizations)
summary(startups$experience_in_fortune_500_organizations)
```
most of the cofouners and founers don't have experience in fortune 100 organizations
## percentage of NA value 

```{r}
NA_Percent = (count(startups$experience_in_fortune_500_organizations=="NA")[2,2]/nrow(startups))*100
NA_Percent
...
## drawing a bar chart  
```{r}
ggplot(startups, aes(x=`experience_in_fortune_500_organizations`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Experience in Fortune 500 organization", x = " Experience? ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
# Summary of Experience in Fortune 1000 organizations

```{r}
startups$experience_in_fortune_1000_organizations = as.factor(startups$experience_in_fortune_1000_organizations)
summary(startups$experience_in_fortune_1000_organizations)
```
most of the cofouners and founers don't have experience in fortune 100 organizations
## percentage of NA value 

```{r}
NA_Percent = (count(startups$experience_in_fortune_1000_organizations=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder their previous experience on fortune 1000 organizations is uknown, either to replace the NA value with appropriate value ofthis feature like no experience, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=`experience_in_fortune_1000_organizations`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Experience in Fortune 1000 organization", x = " Experience? ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
```{r}
md.pattern(startups[,67:69])
```
This function explain that, to what extent there is storge relevant information between the two features, which will hekp us in dealing with NA values
the NA value only can have 0 or 1 unlike the other previous explained features whose NA values can be handled differently.

# Summary of Experience in selling and building products

```{r}
summary(startups$experience_in_selling_and_building_products)
```
 
## percentage of NA value 

```{r}
NA_Percent = (count(startups$experience_in_selling_and_building_products=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder their previous experience is uknown.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=`experience_in_selling_and_building_products`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Products selling&building experience \n", x = " Experience Level ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
The barchart explain that mostly the founders' and co-founders' high-level experience were able to success in their startups business.

# Summary of related Experience to venture

```{r}
summary(startups$relevance_of_experience_to_venture)
```
 
## percentage of NA value 

```{r}
NA_Percent = (count(startups$relevance_of_experience_to_venture=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder their venture related experience is uknown.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=relevance_of_experience_to_venture, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Venture experience \n", x = " Related Experience? ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
There is a slight difference between the status of venture related experience as the graph shows those having experience related to venture might success a bit more than who don't.


# Summary of Exposure across the globe
This feature describe if the founder has global experience or not.
```{r}
summary(startups$exposure_across_the_globe)
```
 
## percentage of NA value 

```{r}
NA_Percent = (count(startups$exposure_across_the_globe=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder their products exposurethe globe experience is uknown.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=exposure_across_the_globe, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Exposure across the globe \n", x = " experined Exposured? ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
We have discussed many features related to experinces of founders and co-founders, now will deep into their educations.

# Summary of Highest education

```{r}
summary(startups$highest_education)
```
The summary function shows that most of the co-founders and founders had either master or bachelor degrees.

## percentage of NA value 

```{r}
NA_Percent = (count(startups$highest_education=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder their educations levels is uknown, either to replace the NA value with appropriate value of this feature like not eduicated, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=highest_education, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Education Level \n", x = "Education Level ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
The barchart shows that founershaving PHD degree would probably success in their startups companies, whereas NA values shows failed stauts.

# Summary of Years of education

```{r}
startups$years_of_education = as.factor(startups$years_of_education)
summary(startups$years_of_education)
```
The summary function shows that most of the co-founders and founders had either 2 or 1 years of education

## percentage of NA value 

```{r}
NA_Percent = (count(startups$years_of_education=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder their years educations   is uknown, either to replace the NA value with appropriate value of this feature like not eduicated, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=years_of_education, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Years of Educations \n", x = "Years ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
The barchart shows that for those having 3 yaesr of educations would probably success in their startups companies, whereas NA values shows failed stauts.

Having analyzing the above two features, we can conclude that both of them can be repalced by 1, and refereing to the same meaning.

# Summary of Relevance of education to venture

```{r}
#startups$years_of_education = as.factor(startups$years_of_education)
summary(startups$relevance_of_education_to_venture)
```
The summary function shows that most of the co-founders and founders who have university degree their studies were relevant to venture.

## percentage of NA value 

```{r}
NA_Percent = (count(startups$relevance_of_education_to_venture=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder venture study related is uknown, either to replace the NA value with appropriate value of this feature like not eduicated, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=relevance_of_education_to_venture, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Venture related study \n", x = "Related? ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
No clear insight wether venture related studies will affect the business startups success.
 
# Summary of Degree from a Tier 1 or Tier 2 university?

```{r}
summary(startups$`degree_from_a_tier_1_or_tier_2_university?`)
```
...
```{r}
NA_Percent = (count(startups$`degree_from_a_tier_1_or_tier_2_university?`=="NA")[2,2]/nrow(startups))*100
NA_Percent

```  
There is about `NA_Percent` of the founder and co-founder their University degree stype is uknown, either to replace the NA value with appropriate value of this feature like not eduicated, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=`degree_from_a_tier_1_or_tier_2_university?`, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Tier1/Tier2 University \n degree type", x = "Tier1|Tier2 ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
# summary of Number of  of Research publications
```{r}
summary(startups$number_of__of_research_publications)
```
Most of the founers are not intersting in research publications.

## percentage of NA value 

```{r}
NA_Percent = (count(startups$number_of__of_research_publications=="NA")[2,2]/nrow(startups))*100
 
```  
There is about `NA_Percent` of the founder and co-founder their Research publications intersting is uknown, either to replace the NA value with appropriate value ofthis feature like no experience, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=number_of__of_research_publications, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Research Publication # \n", x = "High|Few ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
No clear insight wether research publication would affect the business startup case.
...
```
The functions show that most of the founers and co-founders have less than 5 recognitions.

## percentage of NA value  
```{r}
na_percent <- as.numeric((count(startups$ `number_of_recognitions_for_founders_and_co-founders`=="NA"))[2,2]/nrow(startups)*100)
summary(startups$`dependent-company_status`[is.na(startups$`number_of_recognitions_for_founders_and_co-founders`)])
```
about `na_percent` of the co-founers' and founers' recognitions numbers are unknown. Moreover, almost all the NA values are failed with regard to company startups case.

## drawing a histogram of  the feature to know its type of distribution which will help us
## in figuring out the best method to deal with NA vales 
```{r, warning=FALSE}
ggplot(startups, aes(x=`number_of_recognitions_for_founders_and_co-founders`, fill=startups$`dependent-company_status`)) + 
  geom_histogram(alpha= 0.2,aes(y=..count..),binwidth = 50)    +
  labs(title = "Founers Recognitions Numer\n", x = "Recognation #", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
...
It's clearly visualized that, this feature has High right skewed distribution, and this is due to the outliers values. In case we are going to build linea classification model then this feature should be normaly distributed.

## Figuring out the outlier and labeld them
```{r, warning=FALSE}
startups %>%  
  mutate(outlier = ifelse(is_outlier(`number_of_recognitions_for_founders_and_co-founders`), `number_of_recognitions_for_founders_and_co-founders`, as.numeric(NA))) %>%
  ggplot(., aes(x = 1,y = `number_of_recognitions_for_founders_and_co-founders`)) +
  geom_boxplot(outlier.color = "red") +
  geom_text(aes(label = outlier), na.rm = TRUE, hjust = -0.3, color="red")
```

the feature median is almost 0, and values above 260 is considered outliers.
 
## counting outlier and finding their percentage
```{r}
outlier = ifelse(is_outlier(startups$`number_of_recognitions_for_founders_and_co-founders`), startups$`number_of_recognitions_for_founders_and_co-founders`, as.numeric(NA))
outlier_count <- sum(!is.na(outlier))
outlier_percent <- (outlier_count/nrow(startups))*100
summary(startups$`dependent-company_status`[!is.na(outlier)])
```
We have `outlier_count` outlier value, which is around `outlier_percnt` of the total values.
In addition, almost all the outlier values are in the success startups range. 

# summary of Number of Renown score
```{r}
summary(startups$renown_score)
```

## list of the values, and their count
```{r}
count(startups$renown_score)
```
There is NA value.
...
## in figuring out the best method to deal with NA vales 
```{r, warning=FALSE}
ggplot(startups, aes(x=renown_score, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y=..count..))    +
  labs(title = "Founder and co-founer renown score\n", x = "Renew Score", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
...
The grapth shows fluctuting in the company startup case alnog different values of renown_score,
The failed percent start to decrease the more scores founerd have then increaed slightly with the values of 4 and 5 score before starting ti increase gradulally again.

## Figuring out the outlier and labeld them
```{r, warning=FALSE}
startups %>%  
  mutate(outlier = ifelse(is_outlier(renown_score), renown_score, as.numeric(NA))) %>%
  ggplot(., aes(x = 1,y = renown_score)) +
  geom_boxplot(outlier.color = "red") +
  geom_text(aes(label = outlier), na.rm = TRUE, hjust = -0.3, color="red")
```
...
# Summary of Renowned in professional circle

```{r}
summary(startups$renowned_in_professional_circle)
```

```{r}
count(startups$renowned_in_professional_circle)
# declaring a variable to save the value of renowned when its 500
renowed_500 <- as.numeric(count(startups$renowned_in_professional_circle==500)[2,2])
renowed_500_percent <- (renowed_500/nrow(startups))*100
 
```
The count function clearly shows the significant difference between the values in this feature,
almost `renowed_500_percent` of this feature values is 500, while the remainig percentage is distrubuited between 26 till 480 and NA values. Therefore, we can convert this feature to categorical type with two valyes; yes when renowed is 500 and No otherwise.

## percentage of NA value  
```{r}
na_percent <- as.numeric((count(startups$renowned_in_professional_circle=="NA"))[2,2]/nrow(startups)*100)
summary(startups$`dependent-company_status`[is.na(startups$renowned_in_professional_circle)])
```
about `na_percent` of the co-founers' and founers' recognitions numbers are unknown. Moreover, almost all the NA values are failed with regard to company startups case.
...
## in figuring out the best method to deal with NA vales 
```{r, warning=FALSE}
ggplot(startups, aes(x=renowned_in_professional_circle, fill=startups$`dependent-company_status`)) + 
  geom_histogram(alpha= 0.2,aes(y=..count..),binwidth = 50)    +
  labs(title = "Founers Recognitions Numer\n", x = "Recognation #", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
...
The graph shows how much the feature distrubution is HIGH-left skewed.

## Figuring out the outlier and labeld them
```{r, warning=FALSE}
startups %>%  
  mutate(outlier = ifelse(is_outlier(renowned_in_professional_circle), renowned_in_professional_circle, as.numeric(NA))) %>%
  ggplot(., aes(x = 1,y = renowned_in_professional_circle)) +
  geom_boxplot(outlier.color = "red") +
  geom_text(aes(label = outlier), na.rm = TRUE, hjust = -0.3, color="red")

```

# Summary of  top_management_similarity

```{r}
summary(startups$top_management_similarity)
```
 
## percentage of NA value 

```{r}
NA_Percent = (count(startups$top_management_similarity=="NA")[2,2]/nrow(startups))*100
 
```  
There is about `NA_Percent` of the founder and co-founder their management similarity is uknown, either to replace the NA value with appropriate value ofthis feature like no experience, or leave it as it is.

## drawing a bar chart  
```{r}
ggplot(startups, aes(x=top_management_similarity, fill=startups$`dependent-company_status`)) + 
  geom_bar(aes(y = ..count../sum(..count..)))   +   scale_y_continuous(name = "%", labels=scales::percent) +
  labs(title = "Top Management Similarity \n", x = "Management Similarity Level ", y = "Count", fill= "Success|Failed\n ") +
  scale_fill_manual(labels = c("FAILED", "SUCCESS"), values = c("red", "green")) +
  theme_bw() +
  theme(axis.text.x=element_text(size=14), axis.title.x=element_text(size=16),
        axis.text.y=element_text(size=14), axis.title.y=element_text(size=16),
        plot.title=element_text(size=20, face="bold", color="darkgreen"))
```
Whatever the similarity management level is, it's extremely likely the business startup status will success.
