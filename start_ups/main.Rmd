...
## Factor Variables
Let us write a generel method to handle factor variables

```{r to_factor}
to_factor<- function(index) {
  # processing column
names(startups)[index]
...
# Convert to factor
startups[[index]] <- as.factor(startups[[index]])
# Know the levels
levels(startups[[index]])
# Get basic statistics
summary(startups[[index]])
}
```
Now let us construct factor of factor variables and convert them

```{r convert_factors}
factor_cols <- c(2, 12, 16:17, 24, 26:65, 67, 71, 73, 75:87, 89:91, 93, 97, 100:101)
map(factor_cols, to_factor)
```

## Numeric Variables
Let us write a generel method to handle numeric variables
...
startups[[index]][startups[[index]] == ''] <- NA
# Convert to numeric
startups[[index]] <- as.numeric(startups[[index]])
# Get basic statistics
summary(startups[[index]])
}
```
...
map(numeric_cols, to_numeric)
```

## date Variables
Let us write a generel method to handle date variables
...
startups[[index]][startups[[index]] == ''] <- NA
# Convert to numeric
startups[[index]] <- mdy(startups[[index]])
# Get basic statistics
summary(startups[[index]])
}
```
